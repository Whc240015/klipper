...
class ZCalibrationHelper:
    def __init__(self, config):
        ...
        if self.endstop_pin is not None:
            ...
            name = "z_calibration_endstop"
            self.endstops.append((self.mcu_endstop, name))
            # query_endstops = self.printer.load_object(config, 'query_endstops')
            self.query_endstops.register_endstop(self.mcu_endstop, name)
            self.axis = config.get('run_axis').lower()
    ...
    def handle_connect(self):
        ...
        if self.endstop_pin is None:
            ...
            self.z_endstop = EndstopWrapper(self.mcu_endstop)
    ...
    def cmd_CALIBRATE_Z(self, gcmd):
        ...
        if self.bed_site is not None:
            ...
            bed_site = (EMPTY_POSITION, EMPTY_POSITION)
...
class CalibrationState:
    ...
    def _save_offset(self, offset):
        name = self.probe.cmd_helper.name
        z_offset = offset
        configfile = self.helper.printer.lookup_object('configfile')
        configfile.set(name, 'z_offset',
                       "%.3f" % (z_offset,))
        ...
        try:
            ...
            try:
                ...
                switch_zero = self._probe_on_site(self.z_endstop,
                                                  ...
                                                  check_probe=True)
                # probe bed position
                if ((bed_site[0] != EMPTY_POSITION) and (bed_site[1] != EMPTY_POSITION)):
